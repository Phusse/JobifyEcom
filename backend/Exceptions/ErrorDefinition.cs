...
/// </summary>
/// <param name="Id">Unique machine-readable error code (e.g. <c>"AUTH_INVALID_SESSION"</c>).</param>
...
public record ErrorDefinition(string Id, int HttpStatus, string Title, string[] Details)
{
	/// <summary>
	/// Returns a copy with <paramref name="title"/> replacing the existing <see cref="Title"/>.
	/// </summary>
	public ErrorDefinition WithTitle(string title) => this with { Title = title };

	/// <summary>
	/// Returns a copy with <paramref name="suffix"/> appended to the existing <see cref="Title"/>.
	/// </summary>
	public ErrorDefinition AppendTitle(string suffix) => this with { Title = $"{Title}{suffix}" };

	/// <summary>
	/// Returns a copy with <paramref name="details"/> replacing existing <see cref="Details"/>.
	/// </summary>
	public ErrorDefinition WithDetails(params string[] details) => this with { Details = details };
